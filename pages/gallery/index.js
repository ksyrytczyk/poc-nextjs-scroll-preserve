...
export default function Gallery() {
    ...
    function loadMore() {
        const moreImages = Array.from({ length: 50 }, (x, i) => {
            const label = `${i + galleryData.length}-${Math.random().toString().slice(2,10)}`;
            return `https://via.placeholder.com/800x450?text=${label}`;
        });
        setGalleryData([...galleryData, ...moreImages]);
    }
    ...
    return (
        <>
            ...
            <button style={buttonStyle} onClick={() => loadMore()}>Load more</button>
        </>
    );
}
